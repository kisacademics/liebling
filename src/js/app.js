...
$(document).ready(() => {
  ...
  const showNotification = (typeNotification) => {
    const $notification = $(
      `.js-alert[data-notification="${typeNotification}"]`
    );
  ...
  function swapLogo(isDark) {
    if (typeof gh_dark_mode_logo != "undefined") {
      if (!$(".m-logo .only-dark").length) {
        var darkModeImage =
          '<img class="only-dark" src="' + gh_dark_mode_logo + '">';
        $(".m-logo").prepend(darkModeImage);
      }

      if (isDark) {
        $(".m-logo img:not(.only-dark)").hide();
        $(".m-logo img.only-dark").show();
      } else {
        // Swap logo back
        $(".m-logo img:not(.only-dark)").show();
        $(".m-logo img.only-dark").hide();
      }
    } else {
      // do nothing
    }
  ...
  if (currentSavedTheme) {
    ...
    swapLogo(currentSavedTheme === "dark");
  } else {
    if (isDarkMode()) {
      $toggleDarkMode.attr("checked", true);
      swapLogo(true);
